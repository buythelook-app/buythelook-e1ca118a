...
function isValidProductForOccasion(product, occasion) {
  ...
  if (occ === "workout") {
    // HARD REJECT: Formal/Party items (2+ keywords triggers rejection)
    const hardReject = [
      "beaded",
      "sequin",
      "rhinestone",
    ...
    // Expanded workout accept keywords with stronger emphasis
    const workoutAccept = [
      "athletic",
      "sport",
      "gym",
    ...
    const hasPerformanceFabric = /stretch|elastic|fleece|cotton\s*blend|tech|performance|moisture|breathable|wicking|spandex|lycra|polyester|nylon/i.test(combined)
    const isBottomWithPerformance = /\b(trouser|pant|short)\b/i.test(combined) && hasPerformanceFabric && !(/leather|dress|formal|cocktail|satin|velvet/i.test(combined))
    
    const hasRejectKeyword = rejectCount > 0

    return (hasWorkoutKeyword || isBottomWithPerformance) && !hasRejectKeyword
  }

  if (occ === "work" || occ === "business") {
    // HARD REJECT: Casual/Athletic items
    const rejectCasual = [
      "sweatpant",
      "jogger",
      "gym shorts",
    ...
    // HARD REJECT: Party items
    if (/\b(sequin|beaded|rhinestone|glitter|metallic|party|club|sparkle)\b/i.test(combined)) return false
  }

  if (occ === "party" || occ === "formal") {
    // HARD REJECT: Athletic/Casual items
    const rejectCasual = [
      "sweatpant",
      "jogger",
      "gym shorts",
...
export async function searchProductsByCategories(profile) {
  ...
  try {
    ...
    if (USE_VECTOR_SEARCH) {
      ...
      // CRITICAL FIX: Apply occasion validation to vector search results
      ...
      // CRITICAL FIX: If workout bottoms are insufficient, query database directly
      if (occasion === 'workout' && bottoms.length < 10) {
        console.log(`\n🚨 WORKOUT BOTTOMS EMERGENCY FALLBACK: Only ${bottoms.length} found, querying database directly...`)
        
        try {
          // FIX: Expand budget to 0.5x min and 3x max for workout (athletic wear is expensive)
          const expandedMin = Math.floor(categoryBudgets.bottoms.min * 0.5)
          const expandedMax = Math.ceil((categoryBudgets.bottoms.max || 999) * 3)
          
          console.log(`   💰 Expanding budget for workout: $${categoryBudgets.bottoms.min}-${categoryBudgets.bottoms.max} → $${expandedMin}-${expandedMax}`)
          
          const { data: workoutBottoms, error } = await supabase
            .from("zara_cloth_scraper")
            .select("*")
            .eq('outfit_category', 'bottoms')
            .gte('price', expandedMin)
            .lte('price', expandedMax)
            .limit(300) // Get even more to have options after filtering
          
          if (!error && workoutBottoms && workoutBottoms.length > 0) {
            console.log(`   ✅ Found ${workoutBottoms.length} workout bottoms from direct query`)
            console.log(`   📋 Sample products: ${workoutBottoms.slice(0, 5).map(p => p.product_name).join(', ')}`)
            
            const beforeValidation = workoutBottoms.length
            const formattedBottoms = workoutBottoms
              .filter(p => {
                const isValid = isValidProductForOccasion(p, occasion)
                if (!isValid && beforeValidation <= 15) {
                  console.log(`   ❌ Rejected: ${p.product_name}`)
                }
                return isValid
              })
              .slice(0, 50) // Increased from 40 to get more options
              .map((p, index) => {
                const allImgs = extractAllImageUrls(p.images || p.image)
                const validImgs = allImgs.filter((img) => img && img !== "/placeholder.svg" && img.startsWith("http"))
            ...
            bottoms = formattedBottoms
            console.log(`   ✅ Replaced with ${bottoms.length} direct database workout bottoms`)
          }
        } catch (dbError) {
      ...
      // CRITICAL FIX: Mark if emergency fallback was used for bottoms
    ...
    // CRITICAL FIX: Don't run traditional search for bottoms if emergency fallback was used
    const usedEmergencyBottoms = occasion === 'workout' && bottoms.length > 0 && bottoms.length < 10
    ...
    if (needsFallback) {
      ...
      // CRITICAL FIX: Don't run fallback for bottoms if emergency fallback already found some
