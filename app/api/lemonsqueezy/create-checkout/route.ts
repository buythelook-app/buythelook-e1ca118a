...
export async function POST(request: Request) {
  console.log("[] LemonSqueezy: Creating checkout session for links unlock")

  try {
    const { amount, outfitId, description, type, userId } = await request.json()

    console.log("[] LemonSqueezy: Amount:", amount, "cents ($" + amount / 100 + ")")
    console.log("[] LemonSqueezy: Type:", type || "outfit_unlock")
    console.log("[] LemonSqueezy: Outfit ID:", outfitId)
    console.log("[] LemonSqueezy: User ID:", userId)

    if (!userId) {
      console.error("[] LemonSqueezy: Missing user ID")
      return NextResponse.json({ error: "User ID is required" }, { status: 400 })
    }

    if (!outfitId && type === "outfit_unlock") {
      console.error("[] LemonSqueezy: Missing outfit ID for outfit unlock")
    ...
    console.log("[] LemonSqueezy: Checkout URL created")
